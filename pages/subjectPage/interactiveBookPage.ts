...
class InteractiveBookPage extends BasePage {
    ...
    constructor() {
        ...
        this.addBookButton = new Button('.mat-drawer .flex-search-btn-holder button', 'Interactive book: add new book button', { intoIFrame: true });

        this.newBookTitleInput = new TextArea('.cdk-overlay-pane .ck-editor__editable', 'Interactive book: new book title text area', { intoIFrame: true });
        this.newBookTitleSaveButton = new Button('.mat-dialog-actions button', 'Interactive book: save new book button', {
            text: 'Сохранить',
            intoIFrame: true
        });

        this.bookTopic = (topicTitle: string) => new Select('li.mat-tree-node', 'Interactive book: book topic', {
            text: topicTitle,
            intoIFrame: true
        });
    }
